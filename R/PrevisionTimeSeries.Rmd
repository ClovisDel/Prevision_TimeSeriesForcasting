...
additive : Ts = Trend + Seasonal + Random

Multiplicative : Ts = Trend \* Seasonal \* Random
...
-   Saisonnalité $St$ : décrit un pattern qui se reproduit sur un intervale de temps régulier (ici une année), c'est à dire que chaque année, le même pattern est présent.
...
On peut également regarder la distribution des résidus qui ici respecte une courbe gaussienne centrée en 0 avec quelques outliers. 
...
En superposant les différentes années on peut oberservé une saisonnalité qui se répète chaque année. 
...
- Juillet - Aout (la période estivale)
...
Concrètement si on a une MAPE de valeur 6% cela signifique la différence moyenne entre la prédiction et la valeur réelle est de 6%.
...
Une MM 2x4 signfie une MM d'ordre 4 suivi d'une MM d'ordre 2. Concrètement, les 5 premières valeurs de la série chronologique sont 245900, 238000, 263227,277991 et 286691. Les deux premiers termes de la MM d'ordre 4 sera
...
Soit la Moyenne mobile 2x12 $\hat{T}_{t} = \frac{1}{32}y_{t-6} + \frac{1}{32}y_{t-5} + \frac{1}{16}y_{t-4} + \frac{1}{16}y_{t-3} + \frac{1}{16}y_{t-2} + \frac{1}{8}y_{t-1} +\frac{1}{4}y_{t} + \frac{1}{8}y_{t+1} +\frac{1}{16}y_{t+2} + \frac{1}{16}y_{t+3} + \frac{1}{16}y_{t+4} + \frac{1}{32}y_{t+5} + \frac{1}{32}y_{t+6}.$
...
Nous avons vu que la combinaison de moyennes mobiles produisait des moyennes mobiles pondérés, la combinaison que nous avons utilisés produisait les poids suivants $\left[\frac{1}{32},\frac{1}{32},\frac{1}{16},\frac{1}{16},\frac{1}{16},\frac{1}{8},\frac{1}{4},\frac{1}{8},\frac{1}{16},\frac{1}{16},\frac{1}{16},\frac{1}{32},\frac{1}{32}\right]$
...
L'idée de la méthode de lissage exponentielle est une extension de la méthode naîve (naivm vu précédement). Le principe étant de fournir une prévision qui depend de moyenne pondérées d'observations passées, les poids diminuant de façon exponentielle à mesure que les observations vieillissent. 

Plus simplement, les observations les plus récentes ont un poid plus important que les observations les plus anciennes.
...
*Le modèle ARMA (Autoregressive Moving Average Model) est définit par l'idée de décrire une séries temporelles en deux polynômes. Le premier étant pour l'auto regréssion et le second pour la moyenne mobile. 
ARMA(p,q) où : 
  - p est l'ordre du polynôme autorégressif,
  - q est l'ordre du polynôme de moyenne mobile
L'équation est données par 
avec : 
    [ Y A UNE IMAGE LA ]
...
Une différentiation correspond à rétirer les tendances -\> tendance linéaire : une différenciation -\> tendance quadradique : deux différenciations
...
Préparation des données: - format des dates - colonne du temps name : ds - colonne de la mesure quantitative name : y
...
Un réseau LSTM (Long Short Term Memory) est un type particulier de réseau de neuronnes récurrent qui est capable d'apprendre les dépendances à long terme. Il a été introduit pas Hochreiter & Schmidhuber en 1997. 

L'idée d'un réseaux récurrent est de mettre en place des boucles qui permettent aux informations de persister. Ils sont représentés par une chaine de modules répétitifs de réseau de neuronnes. 

Chaque module est traversé un à un en faisant évoluer son état qui peut se voir ajouter ou supprimer de l'informations, ces changements sont régulées par ce qu'on appelle des "portes". Elle est définit par couche de neuronnes sigmoïde et d'opération de multiplication ponctuelle qui laisse passer ou non l'information. La fonction Sigmoïde produit des nombres entre 1 et 0 ce qui définit la quantité de chaque composant qui doit passer.  
...
Puis en transforme en array 3D car le modèle LSTM prend un tensor de format 3D [samples, timesteps, features] samples : nbr d'observation par batchs timesteps : lag features : nbr de valeur predites
...
On a 50 résultats / prédictions par input donc \> transforme pour une seule prédiciton
