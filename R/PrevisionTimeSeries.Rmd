...
la tendance
Prédiction sur les données futurs.
```{r}
tendance=predict(Regression)
Annees=as.numeric(Annees)
futur=seq(max(Annees)+1/12,length=36,by=1/12)  
tendance2=predict(Regression, newdata=data.frame(X=futur)) 
```
...
Prediction3 =predict( Regression2, newdata= data.frame(mois=moisfutur))

```

Calculons une région de confiance avec l'erreur d'ajustement
```{r}
E=residuals(Regression2)
hist(E)
1.96*sqrt(var(E))
```
...
```{r}
Buys_ballot_plot <- plot(data_ts_train) 
lines(Annees,tendance,col="blue",lwd=1)  
lines(futur,tendance2,col="red")
lines(Annees,tendance+prediction2,col="green",lwd=1)


lines(futur,tendance2+Prediction3,col="blue")

 polygon(c(futur,rev(futur)),
 c(tendance2+Prediction3-1.96*sqrt(var(E)),
 rev(tendance2+Prediction3+1.96*sqrt(var(E)))),
 col="purple",border=NA)
```
 Nous avons réussi à ajuster une droite de régression.
